...
if __name__ == "__main__":
    ...
    print(cyan('##################################################',bold=True))
    a=input('Presse Enter to Continue')    
    print(cyan('\n##################################################',bold=True))
    print(cyan('#                                                #',bold=True))
    ...
    for ip_address in infected_machine_list:
        response_actions_url=f"/respond/trigger/9c99aefa-64be-4a3d-884e-d4f14e60eebc/01YD3Z1A74H553WXOpHSOD0cJVN1fw1ik0T?observable_type=ip&observable_value={ip_address}&action-id=01YD3Z1A74H553WXOpHSOD0cJVN1fw1ik0T"
        result=xdr_trigger_response_action(env.XDR.get("host"),response_actions_url)
        print(green(result,bold=True))
    if result=='':
        sys.exit()     
    ...
    # PUT YOUR CODE HERE UNDER
    result=''     
    ctr_observables={'value':'retdemos.com','type':'domain'}
    response_actions = ctr_response_actions(ctr_access_token,ctr_observables)
    print('available response actions are :\n',cyan(response_actions,bold=True))
    print(red(f'Line number : {str(env.get_line())}',bold=True))        
    a=input('\nType Enter to Continue :')    
    
    malicious_domain="retdemos.com"
    response_actions_url=f"/respond/trigger/4b7a22cb-09a2-4b35-9b62-199bb55329c6/block?observable_type=domain&observable_value={malicious_domain}"
    result=xdr_trigger_response_action(env.XDR.get("host"),response_actions_url)
    print(green(result,bold=True))    
    if result=='':
        sys.exit()     
    ...
    print('available response actions are :\n',cyan(response_actions,bold=True))
    a=input('\nType Enter to Continue :')  
    malicious_sha256="b1380fd95bc5c0729738dcda2696aa0a7c6ee97a93d992931ce717a0df523967"
    response_actions_url=f"/respond/trigger/9c99aefa-8b06-4df8-96f4-a89e3f2556ef/01GWHRNGESXD03H9ZF47jbf3ZzJKez1F0Ej?observable_type=sha256&observable_value={malicious_sha256}"
    result=xdr_trigger_response_action(env.XDR.get("host"),response_actions_url)
    if result == 200:
